...
def calculate_color_parameters(spd_data, cmf_file='Data/ciexyz31.csv', norm_Y=100):
    ...
    X = np.trapz(merged['power'] * merged['x_bar'], dx=delta_lambda)
    Y = np.trapz(merged['power'] * merged['y_bar'], dx=delta_lambda)
    Z = np.trapz(merged['power'] * merged['z_bar'], dx=delta_lambda)
